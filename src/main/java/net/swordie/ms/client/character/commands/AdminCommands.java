...
public class AdminCommands {
    ...
    @Command(names = {"forceevent"}, requiredType = Admin)
...
//            outPacket.encodeArr(new byte[Integer.parseInt(args[1])]);
    ...
    @Command(names = {"getitem", "item"}, requiredType = GameMaster)
    ...
    //displaying the online players by channel.
    @Command(names = {"online"}, requiredType = Admin)
    public static class OnlineCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            Client client = chr.getClient();
            int total = 0;
            for (Channel ch : client.getWorld().getChannels()) {
                int size = ch.getChars().size();
                if (size != 0){
                    total += size;
                    String s = "(Channel " + ch.getChannelId() + " Online: " + size + ") : ";
                    if (ch.getChars().size() < 50) {
                        Map<Integer, Char> players = ch.getChars();
                        for(Map.Entry<Integer, Char> c : players.entrySet()) {
                            s += c.getValue().getName() + ", ";
                        }
                        chr.chatMessage(Expedition,s.substring(0, s.length() - 2));
                    }
                }
            }
            chr.chatMessage(Expedition,"Players online: " + total, 300);
        }
    }

    //displaying the memory heap size of the server.
    @Command(names = {"heap"}, requiredType = Admin)
    public static class heapCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            MemoryUsage memory = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage();
            chr.chatMessage(Expedition,"used = " + memory.getUsed());
            chr.chatMessage(Expedition,"committed = " + memory.getCommitted());
            chr.chatMessage(Expedition,"max = " + memory.getMax());
        }
    }

    //warping to player command.
    @Command(names = {"warpto"}, requiredType = Admin)
    public static class WarpToCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            if (args.length < 1){
                chr.chatMessage(Expedition,"wrong syntax, !warpto <playerName>");
                return;
            }
            String name = args[1];
            Char player = chr.getWorld().getCharByName(name);
            if (player != null && player.isOnline()){
                chr.warp(player.getFieldID());
            }
            else{
                chr.chatMessage(Expedition,"Unknown player: " + args[1]);
            }
        }
    }

    //warping the players to you.
    @Command(names = {"warphere"}, requiredType = Admin)
    public static class WarpHereCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            if (args.length < 1){
                chr.chatMessage(Expedition,"wrong syntax, !warphere <playerName>");
                return;
            }
            String name = args[1];
            Char player = chr.getWorld().getCharByName(name);
            if (player != null && player.isOnline()){
                player.warp(chr.getFieldID());
            }
            else{
                chr.chatMessage(Expedition,"Unknown player: " + args[1]);
            }
    ...
    @Command(names = {"getmap"}, requiredType = Admin)
    public static class getMapCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            chr.chatMessage(Expedition,"" + chr.getFieldID());
        }
    }

    @Command(names = {"exp"}, requiredType = Admin)
    public static class ExpCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            if (args.length < 1)
    ...
    public static class JobInfoCommand extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            chr.chatMessage(Expedition, "Job: " + chr.getJob() + "\nSubJob: " + chr.getSubJob());
            chr.chatMessage(Expedition, "FinalJob: " + chr.getFinalJob());
        }
    }
